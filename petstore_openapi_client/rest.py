...
from petstore_openapi_client.logging_utils import (
    build_summary,
    pretty_log,
    redact_headers,
...
class RESTClientObject:
    ...
    async def request(
        ...
        request_log = {
            ...
            "body": request_body_for_log(args.get("data"), headers),
        }
        logger.debug(
            pretty_log(
                build_summary("Request", method, args["url"]),
                request_log,
            )
        )
        ...
        if _preload_content:
            ...
            response_log = {
                ...
                "body": response_body_for_log(r.data),
                "url": args["url"],
            }
            logger.debug(
                pretty_log(
                    build_summary("Response", method, args["url"], r.status),
                    response_log,
                )
            )
